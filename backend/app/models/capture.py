...
class ChangeDetection(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    location_id = Column(UUID(as_uuid=True), ForeignKey("locations.id"), nullable=False, index=True)
    before_capture_id = Column(UUID(as_uuid=True), ForeignKey("captures.id"), nullable=False)
    after_capture_id = Column(UUID(as_uuid=True), ForeignKey("captures.id"), nullable=False)
